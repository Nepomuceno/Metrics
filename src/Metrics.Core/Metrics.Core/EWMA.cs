...
namespace Metrics.Core
{
    ...
    public class EWMA
    {
        ...
        public static EWMA operator +(EWMA ewma1, EWMA ewma2)
        {
            if (Math.Abs(ewma1.TimeInterval - ewma2.TimeInterval) > 0.000001)
            {
                throw new Exception("Impossible to sum EWMA with diferente time intervals");
            }
            return new EWMA(ewma1.Alpha,ewma1.TimeInterval,ewma1.Rate + ewma2.Rate);
        }
    }
}